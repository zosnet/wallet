...
// Filter ZOS 0.9.2+ import keys export file so that it will include only private keys
// that may be found in the ZOS 2.0 genesis block.
// Dependencies:
// ./bloom_zos.dat (1,048,576 bytes) sha1 4d80faa41a5e868899febdc9dab48d1f2d567487992810cf0532f3c0ee2b266c
// nodejs, npm, and: npm install

/*
* ./bloom.dat is from zos/graphene/programs/genesis_util/create_bloom_filter.py
* The bloom filter was created with a genesis containing ZOS prefixed keys.  Create
...
var graphenejs = require("zosjs");
var h = graphenejs.hash;
var key_utils = graphenejs.key;
const chainPrefix = "ZOS";

fs.readFile("bloom_zos.dat", function(err, data) {
    if (err) throw err;
    console.error(
        "bloom_zos.dat (" + data.length + " bytes)",
        "sha1",
    ...
    stdin.on("end", function() {
        ...
        for (let a = 0; a < wallet.account_keys.length; a++) {
            ...
            for (var k = keys.encrypted_private_keys.length - 1; k >= 0; k--) {
                ...
                if (/^NPT/.test(key)) key = chainPrefix + key.substring(3);
