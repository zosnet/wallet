...
class Asset extends React.Component {
    ...
    _assetType(asset) {
        let assetType =
            "bitasset" in asset
                ...
                : "Simple";
        //获取asset的属性
        let propertys = assetUtils.getAssetProperty(asset.uasset_property);
        for (let property in propertys) {
            if (propertys[property]) {
                let propertyTxt = counterpart.translate(
                    `explorer.asset.summary.${property}`
                );
                assetType = assetType.concat(" / ").concat(propertyTxt);
            }
        }
        return assetType;
    }
    ...
    renderPriceFeed(asset, sortedCallOrders) {
        ...
        let feedFlag = "";
        if (flagBooleans["witness_fed_asset"]) {
            //见证人喂价
            feedFlag = counterpart.translate(
                "explorer.asset.price_feed.witness_feed"
            );
        } else if (flagBooleans["committee_fed_asset"]) {
            //委员会喂价
            feedFlag = counterpart.translate(
                "explorer.asset.price_feed.committee_feed"
            );
        } else {
            //指定喂价人
            feedFlag = counterpart.translate(
                "explorer.asset.price_feed.feeder"
            );
        }
                ...
                >
                    <tbody>
                        <tr>
                            <td>
                                {" "}
                                <Translate content="explorer.asset.price_feed.feed_flag" />{" "}
                            </td>
                            <td> {feedFlag} </td>
                        </tr>
                        <tr>
    ...
    render() {
        ...
        return (
            ...
            >
                <div className="grid-block page-layout zos-card-bg">
