...
class BitAssetOptions extends React.Component {
    ...
    render() {
        ...
        return (
            <div className="small-12 grid-content" style={{paddingTop: 20}}>
                <label>
...
class AccountAssetCreate extends React.Component {
    ...
    resetState(props) {
        ...
        let isInnerAsset = false;

        //Router中的Link to 传递过来的state参数
        //console.log("props.location:",props.location)
        if (props.location.state && props.location.state.isInnerAsset) {
            isInnerAsset = props.location.state.isInnerAsset; //是否内盘资产(内盘资产发行人是1.2.0)
            isBitAsset = true; //内盘资产一定是智能币
        }
    ...
    _createAsset(e) {
        ...
        let pay_proposal_account_id;
        let account_id = account.get("id");
        //如果创建内盘资产则发行人为1.2.0
        if (isInnerAsset) {
            account_id = "1.2.0";
            pay_proposal_account_id = this.props.account.get("id");
        }
        console.log("pay_proposal_account_id:", pay_proposal_account_id);
    ...
    _onToggleBitAsset() {
        ...
        if (!this.state.isBitAsset) {
            ...
            this.state.isInnerAsset = false; //非智能资产一定不是内盘资产
        }
    ...
    _onToggleInnerAsset() {
        this.state.isInnerAsset = !this.state.isInnerAsset;
        if (this.state.isInnerAsset) {
            this.state.isBitAsset = true; //内盘资产一定是智能资产
            let {flagBooleans, permissionBooleans} = this._getPermissions(
                this.state
            );
            this.state.flagBooleans = flagBooleans;
    ...
    render() {
        let {globalObject, core, account} = this.props;
        let {
            errors,
            isValid,
            update,
        ...
        let maxValueStrArr = Array.from(GRAPHENE_MAX_SHARE_SUPPLY.toString());
        //倒数precision位之前插入点号"."
        let precision = this.state.update.precision;
        if (precision > 0) {
            maxValueStrArr.splice(-precision, 0, ".");
        }
        let maxValueInl = counterpart.translate(
            "account.user_issued_assets.max_value"
        );
        //提示语组装
        const maxValueTip = `(${
            update.symbol
        } ${maxValueInl}: ${maxValueStrArr.join("")})`;
        //console.log(maxValueTip)
        ...
        return (
            ...
            >
                ...
                <div
                    className="zos-block-content-header zos-card-bg"
                    style={{
                        background: "#fff"
                    }}
                >
                    <Translate component="h4" content="header.create_asset" />
                    <div
                        style={{
                            position: "absolute",
                            right: 20,
                                            ...
                                            {isLTM ? ( //只有终身会员能够创建内盘资产，这里才显示
                                                <tr>
                                                    ...
                                                    >
                                                        <Translate
                                                            content={
                                                                "account.user_issued_assets.inner_asset"
                                                            }
                                                        />
                                                        :
                                                    </td>
                                                    ...
                                                    >
                                                        <div
                                                            ...
                                                            onClick={this._onToggleInnerAsset.bind(
                                                                this
                                                            )}
                                                        >
                                                            <input
                        ...
                        <Tab title="account.user_issued_assets.description">
                            ...
                            >
                                ...
                                >
                                    ...
                                    <Translate
                                        component="label"
                                        content="account.user_issued_assets.real_asset"
                                    />
                                    <label>
                                        <input
                                            type="text"
