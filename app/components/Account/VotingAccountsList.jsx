...
function getWitnessOrCommittee(type, acct) {
    ...
    if (type === "witness") {
        ...
        account = ChainStore.getBudgetMemberById(acct.get("id"));
    }

    url = account ? account.get("url") : url;
    votes = account ? account.get("total_votes") : votes;
    enable = account ? account.get("enable") : enable;
    return {
        url,
        votes,
...
class AccountItemRow extends React.Component {
    ...
    shouldComponentUpdate(nextProps) {
        return (
            ...
            nextProps.isLost !== this.props.isLost
        );
    }
    ...
    render() {
        let {account, type, action, isActive, isLost} = this.props;
        ...
        let trans_enable_lost = null;
        if (isLost && isActive) {
            trans_enable_lost = (
                <span>
                    /<Translate content="account.votes.enable_lost" />
                </span>
            );
        }
        return (
            <tr className={isSupported ? "" : "unsupported"}>
                ...
                >
                    {!this.props.proxy ? (
                        <input
                            type="checkbox"
                            checked={isSupported ? true : false}
                        />
                    ) : (
                        <Icon name="locked" />
                    )}
    ...
    render() {
        ...
        let item_rows = this.props.items
            ...
            .map((i, idx) => {
                ...
                let isLost =
                    getWitnessOrCommittee(this.props.type, i).enable ===
