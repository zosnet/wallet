...
class Transfer extends React.Component {
    ...
    //需要计算优惠券:余额-(优惠券 - 手续费>=0? 0: 手续费)   -- 优惠券和手续费不能分开扣
    ...
    _getAvailableAssets(state = this.state) {
        ...
        for (let key in account_balances) {
            ...
            // 法币不允许转账
            let assetObject = ChainStore.getObject(key);
            let uasset_property = assetObject
                ? assetObject.get("uasset_property").toString(2)
                : null;
            if (uasset_property) {
                let uasset_property_array = uasset_property.split("").reverse();
                if (uasset_property_array[0] == 1) {
