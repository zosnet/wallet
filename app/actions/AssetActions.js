...
class AssetActions {
    ...
    createAsset(
        ...
        bitasset_opts,
        description,
        pay_proposal_account_id //如果有值则发提议，如果为空则不发提议
    ) {
        let tr = WalletApi.new_transaction();
        ...
        let transfer_op = tr.get_type_operation("asset_create", operationJSON);
        return dispatch => {
            return tr.update_head_block().then(() => {
                if (pay_proposal_account_id) {
                    tr.add_type_operation("proposal_create", {
                        proposed_ops: [{op: transfer_op}],
                        fee_paying_account: pay_proposal_account_id
                    });
                } else {
    ...
    updateAsset(
        ...
        if (assetChanged) {
            ...
            let updateOp = tr.get_type_operation("asset_update", updateObject);
            proposed_ops.push({op: updateOp});
        }
        ...
        if (
            ...
            let updateBitOp = tr.get_type_operation(
                "asset_update_bitasset",
                bitAssetUpdateObject
            );
            proposed_ops.push({op: updateBitOp});
        }
        ...
        if (
            ...
            let updateFeedProducersOp = tr.get_type_operation(
            ...
            proposed_ops.push({op: updateFeedProducersOp});
        }
        ...
        return tr.update_head_block().then(() => {
            if (pay_proposal_account_id) {
                ...
                proposed_ops.map(oprations => {
                    tr.add_operation(oprations.op);
                });
            }
    ...
    issueAsset(to_account, from_account, asset_id, amount, memo) {
        //console.log("issueAsset:",to_account,from_account,asset_id,amount,memo)
        return ApplicationApi.issue_asset(
            to_account,
            from_account,
            asset_id,
            amount,
    ...
    changeAssetProperty(
        ...
        //console.log( "Property:",issuer, asset_id, uasset_property, uasset_mask);
        var tr = WalletApi.new_transaction();
        let transfer_op = tr.get_type_operation("asset_property", {
            fee: {
                amount: 0,
                asset_id: 0
            },
            issuer, //资产发行人 account  id
            asset_id, //资产id
            uasset_property, //资产属性
            uasset_mask, //掩码
            extensions: []
        });
        //asset_property只能发提议
