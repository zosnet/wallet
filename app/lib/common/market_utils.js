...
import {ChainStore, ChainTypes} from "zosjs/es";
...
const MarketUtils = {
    ...
    parseOrder(order, base, quote, invert = false) {
        ...
        if (order.debt) {
            if (invert) {
                // Price in USD/ZOS, amount should be in ZOS, value should be in USD, debt is in USD
                // buy is in USD, sell is in ZOS
                // quote is USD, base is ZOS
                value = order.debt / quotePrecision;
                amount = this.limitByPrecision(value / price.full, base);
            } else {
                // Price in ZOS/USD, amount should be in USD, value should be in ZOS, debt is in USD
                // buy is in USD, sell is in ZOS
                // quote is USD, base is ZOS
