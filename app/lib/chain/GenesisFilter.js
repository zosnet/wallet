...
try {
    ...
    console.log(
        "WARN: Will be unable to filter ZOS 1.0 wallet imports, did not find assets/bts_genesiskeys_bloom.dat",
        e
    );
}

/**
    This should only be applied to a ZOS 1.0 export file taken on the
    discontinued chain. Any public key string or address (all 5 formats) carried
    over to the ZOS 2.0 genesis block will be in this filter.
...
export default class GenesisFilter {
    ...
    filter(account_keys, status) {
        if (!this.isAvailable()) {
            console.log("WARN: Missing bloom filter for ZOS 0.9.x wallets");
            ...
            try {
                ...
                for (var a = 0; a < account_keys.length; a++) {
                    ...
                    for (
                        ...
                        if (/^NPT/.test(currentKey))
                            currentKey = "ZOS" + currentKey.substring(3);
                        if (this.inGenesis(currentKey)) continue;
                        var addresses = key.addresses(currentKey, "ZOS");
