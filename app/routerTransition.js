...
const willTransitionTo = (
    ...
    // Bypass the app init chain for the migration path which is only used at zos.org/wallet
    if (__DEPRECATED__) {
    ...
    SettingsStore.setApiServer().then(res => {
                ...
                .then(() => {
                    ...
                    return chainStoreResetPromise.then(() => {
                        return Promise.all([
                            ...
                            fetch(`${__BASE_URL__}config.json`).then(reply => {
                                reply.json().then(config => {
                                    if (
                                        config.chain.chain_id ===
                                        Apis.instance().chain_id
                                    ) {
                                        ChainConfig.setPrefix(
                                            config.chain.address_prefix
                                        );
                                        console.log(
                                            "Successful modification of chain ID",
                                            config.chain.chain_id
                                        );
                                    } else {
                                        console.error(
