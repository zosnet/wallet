...
In order to increase your exposure to ZOS and offer liquidity to BitAssets, such
...
The zos network is capable of issuing any amount of any BitAsset and lend
it out to participants given enough collateral.

 * *settlement price*: The price for 1 ZOS as it is traded on external exchanges.
...
The zos network is capable of margin calling those positions that do not
...
offer to buy the collateral is less than the call price (x/ZOS).
...
market rates up to the SQP and close the position. The remaining ZOS of the
...
increase, an additional amount of ZOS is locked as collateral, while reducing
...
particular bitAsset to hand it over to the zos network. After that, the
