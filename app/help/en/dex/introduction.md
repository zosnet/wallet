...
direct exchange of digital goods traded in the zos ecosystem.
...
briefly below. Although the zos DEX comes with all of them, it is up to
...
* **Separation of Powers**: 
  ...
  issuing IOUs and for processing the order book. In zos, order matching
  is performed by the protocol, which is unaware of implications concerning the
  involved assets.
  
* **Global Unified Order Book**:
  Since zos is global, anybody with an internet access can use the DEX for
  trading. This brings the world's liquidity to a single order book for
  decentralized trading.
  
* **Trade Almost Anything**:
  The zos DEX is asset agnostic. Hence you can trade at **any** pair.
...
* **No Limits**:
  The zos protocol is unable to limit your trading experience.
  
* **Decentralized**:
  The DEX is decentralized across the globe. This not only means that there is
  no single point of failure, but it also implies that the zos exchange is
...
* **Fast**:
  In contrast to other decentralized networks, the zos DEX allows for
  real-time trading and is only limited by the speed of light and the size of
  the planet.
  
* **Provable Order Matching Algorithm**:
  What makes the zos DEX unique is the provable order matching algorithm.
...
* **Collateralized Smartcoins**:
  One of the biggest features of zos are its *smartcoins* such as bitUSD,
  ...
  backed by zos' company shares (ZOS) being locked up as collateral and
