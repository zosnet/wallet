...
const CORE_ASSET = "ZOS"; // Setting this to ZOS to prevent loading issues when used with ZOS chain which is the most usual case currently
...
class SettingsStore {
    constructor() {
        this.exportPublicMethods({
            ...
            setApiServer: this.setApiServer.bind(this),
            refreshState4InitError: this.refreshState4InitError.bind(this)
        });
        ...
        let defaults = {
            ...
            themes: ["darkTheme", "lightTheme", "midnightTheme", "zosTheme"],
            passwordLogin: [
    ...
    setApiServer() {
        let configPromise = fetch(`${__BASE_URL__}config.json`);
        return Promise.all([configPromise]).then(reply => {
            return reply[0].json().then(config => {
                return new Promise((resolve, reject) => {
                    let setAPIs = config.settingsAPIs;
                    this.defaultSettings.set(
                        "apiServer",
                        setAPIs.DEFAULT_WS_NODE
                    );
                    this.defaultSettings.set(
                        "faucet_address",
                        setAPIs.DEFAULT_FAUCET
                    );
                    this.settings = Immutable.Map(
                        merge(
                    ...
                    let apiServer = setAPIs.WS_NODE_LIST;
                    let defaults = {
                        apiServer: apiServer
                    };
                    let savedDefaults = ss.get("defaults_v1", {});
                    this.defaults = merge({}, this.defaults, defaults);
                    (savedDefaults.apiServer || []).forEach(api => {
                        let hasApi = false;
                        if (typeof api === "string") {
                            api = {url: api, location: null};
                    ...
                    if (
                        !savedDefaults ||
                        (savedDefaults &&
                            (!savedDefaults.apiServer ||
                                !savedDefaults.apiServer.length))
                    ) {
                        for (let i = apiServer.length - 1; i >= 0; i--) {
                            let hasApi = false;
                            this.defaults.apiServer.forEach(api => {
    ...
    init() {
        return new Promise(resolve => {
            ...
            let coreAsset = coreAssets[this.starredKey] || "ZOS";
